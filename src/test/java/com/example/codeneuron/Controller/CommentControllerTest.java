...
public class CommentControllerTest {
    ...
    public void init(){
        ...
        when(this.commentService.addComment("com.example.test:test()","测试用例代码","node",1)).thenReturn(ResponseVO.buildSuccess());
        when(this.commentService.deleteComment(0,1,"domain")).thenReturn(ResponseVO.buildSuccess());
        when(this.commentService.getComment(1,"edge")).thenReturn(ResponseVO.buildSuccess(new CommentVO(2,"com.example.test:test()","测试用例代码","edge",1)));
//        when(this.commentService.getNodeCommentByName("com.example.test:test()")).thenReturn(ResponseVO.buildSuccess(new CommentVO(2,"com.example.test:test()","测试用例代码","edge")));
    ...
    public void addComment() throws Exception{
        ...
        CommentForm commentForm=new CommentForm("com.example.test:test()","测试用例代码","node",1);
...
//    }
    ...
    public void updateComment() throws Exception{
        MockMvc mockMvc=standaloneSetup(commentController).build();
        CommentVO commentVO=new CommentVO(1,"com.example.test:test()","测试用例代码2","node",1);
